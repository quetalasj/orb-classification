...
class ORBRegDataset(Dataset):

    def __init__(self, root, datasets, sigmoid_function,  transform, combine_data=True):
        root = Path(root)
        images_list = []
        labels_list = []
        features_list = []
        for dataset in datasets:
            ...
            labels_list.append(sigmoid_function(dataset_features))
    ...
    def __getitem__(self, idx):
        ...
        label = torch.tensor(self.labels[idx], dtype=torch.float32)[None]
        if self.transform:
